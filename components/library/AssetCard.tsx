...
import { Star, Trash2, FileText, Film, Eye, Loader2 } from 'lucide-react'
        ...
        >
          ...
          <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button
              ...
              disabled={isPending}
              className="p-1.5 rounded-lg bg-black/60 backdrop-blur hover:bg-red-600"
            >
      ...
      {showPreview && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-8" onClick={() => setShowPreview(false)}>
          <button
            ...
            className="absolute top-4 right-4 w-10 h-10 rounded-full bg-black/60 hover:bg-black/80 flex items-center justify-center text-white text-2xl"
          >
            ×
          </button>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 max-w-2xl">
                ...
                {asset.extractedText && (
                  <div className="mt-4 max-h-96 overflow-y-auto p-4 bg-gray-950/60 border border-gray-800 rounded-lg">
                    ...
                    {asset.extractedText.length > 3000 && <div className="text-xs text-gray-600 mt-2">... ({asset.extractedText.length - 3000} caractères de plus)</div>}
                  </div>
                )}
                <a
                  href={asset.url}
                  ...
                  className="inline-block mt-4 px-4 py-2 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-sm"
                >
                  Ouvrir le document
                </a>
              </div>
