    ...
    <div className="space-y-6 max-w-6xl">
      ...
      {posts.length === 0 ? (
        <div className="bg-gray-900/20 border border-dashed border-gray-700 rounded-2xl p-10 text-center">
          ...
          <Link href={`/studio?client=${client.id}`} className="inline-flex items-center gap-1.5 text-sm text-purple-400 hover:underline mt-3">
            Créer un post
          </Link>
        </div>
      ) : (
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {Object.entries(post.metaPostIds).map(([platform, id]) => (
              <span key={`${platform}-${id}`} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-700 bg-gray-950/50 px-2 py-1 text-[11px] text-gray-300">
