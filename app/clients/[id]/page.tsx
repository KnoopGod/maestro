...
export default async function ClientDetailPage({
  ...
  const [assetSummary, identity, aiStrategy, socialAccounts, clientPosts, clientJobs] =
    await Promise.all([
  ...
  const setupComplete = Boolean(
    client.description && client.brandVoiceTone && contentPillars.length > 0,
  )
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <Link
        href="/clients"
        title="Retourner à la liste complète des clients"
      ...
      <div className="flex items-start gap-5 pb-6 border-b border-gray-800">
        <div
          className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${client.color} flex items-center justify-center text-4xl shadow-lg flex-shrink-0`}
        >
          {client.emoji}
        </div>
        ...
        <div className="flex gap-2 flex-shrink-0">
          <Link
            ...
            className="px-3 py-2 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 hover:opacity-90 active:scale-[0.98] text-white text-sm font-medium flex items-center gap-1.5 transition-all duration-150"
          >
          ...
          <Link
            ...
            className="px-3 py-2 rounded-lg border border-purple-700/40 hover:bg-purple-900/30 hover:border-purple-600/60 text-purple-300 text-sm flex items-center gap-1.5 transition-all duration-150 active:scale-[0.98]"
          >
          ...
          <Link
            ...
            className="px-3 py-2 rounded-lg border border-gray-800 hover:bg-gray-800 hover:border-gray-700 text-gray-300 text-sm flex items-center gap-1.5 transition-all duration-150 active:scale-[0.98]"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        <Link
          ...
          title="Tunnel guidé : unifier, configurer et connecter les réseaux sociaux du client en 5 étapes"
          className="bg-gradient-to-br from-indigo-950/40 to-blue-950/30 border border-indigo-700/30 rounded-xl p-4 hover:border-indigo-500/50 transition-all flex items-center gap-3 group"
        >
        ...
        <Link
          ...
          title="Ajouter photos, vidéos, logos et documents de DA du client"
          className="bg-gradient-to-br from-purple-950/40 to-pink-950/30 border border-purple-700/30 rounded-xl p-4 hover:border-purple-500/50 transition-all flex items-center gap-3 group"
        >
          ...
          <div>
            ...
            <div className="text-[11px] text-gray-500">
              {assetSummary.total} éléments{identity?.stylePrompt ? ' · DA' : ''}
            </div>
          </div>
        </Link>

        <Link
          href={`/clients/${client.id}/agents`}
          title="Voir les agents IA qui ont travaillé pour ce client et leurs livrables"
          className="bg-gray-900/40 border border-gray-800 rounded-xl p-4 hover:border-purple-700/50 transition-all flex items-center gap-3 group"
        >
        ...
        <Link
          ...
          title="Voir les posts planifiés, publiés et en préparation pour ce client"
          className="bg-gray-900/40 border border-gray-800 rounded-xl p-4 hover:border-purple-700/50 transition-all flex items-center gap-3 group"
        >
        ...
        <Link
          ...
          title="Voir les performances des posts publiés et récupérer les insights Meta"
          className="bg-gray-900/40 border border-gray-800 rounded-xl p-4 hover:border-purple-700/50 transition-all flex items-center gap-3 group"
        >
          ...
          <div>
            ...
            <div className="text-[11px] text-gray-500">
              {avgImpact != null ? `Impact ${avgImpact.toFixed(0)}/100` : 'Aucun post publié'}
            </div>
          </div>
        </Link>
        ...
        <Link
          ...
          title="Suivre le coût IA/API, le budget client et la marge de rentabilité"
          className="bg-gray-900/40 border border-gray-800 rounded-xl p-4 hover:border-emerald-700/50 transition-all flex items-center gap-3 group"
        >
        ...
        <Link
          ...
          title="Connecter Facebook, Instagram et les plateformes nécessaires pour publier"
          className="bg-gray-900/40 border border-gray-800 rounded-xl p-4 hover:border-purple-700/50 transition-all flex items-center gap-3 group"
        >
        ...
        <Link
          ...
          title="Préparer un bilan mensuel imprimable ou exportable en PDF pour le client"
          className="bg-gradient-to-br from-emerald-950/40 to-teal-950/30 border border-emerald-700/30 rounded-xl p-4 hover:border-emerald-500/50 transition-all flex items-center gap-3 group"
        >
      ...
      <div className="bg-gray-900/40 border border-gray-800 rounded-2xl p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
          ...
          <StartupStep
            ...
            detail={
              firstPostReady
                ? `${clientPosts.length} post${clientPosts.length > 1 ? 's' : ''}`
                : 'Créer le premier'
            }
            href={`/studio?client=${client.id}`}
          />
      ...
      {identity && identity.stylePrompt && (
        <div className="bg-gradient-to-r from-purple-950/40 to-pink-950/30 border border-purple-700/30 rounded-2xl p-5 flex items-start gap-4">
          ...
          <Link
            ...
            className="px-3 py-1.5 text-xs rounded-lg border border-purple-700/40 text-purple-300 hover:bg-purple-900/30 hover:border-purple-600/60 transition-all duration-150 active:scale-[0.98]"
          >
            Gérer →
          </Link>
        </div>
            ...
            <dl className="space-y-3 text-sm">
              <div>
                ...
                <dd className="text-gray-200">
                  {client.brandVoiceTone || (
                    <span className="text-gray-600 italic">Non défini</span>
                  )}
                </dd>
              </div>
              <div>
                <dt className="text-[11px] uppercase tracking-wider text-gray-500 mb-1">
                  Mots-clés
                </dt>
                <dd className="text-gray-200">
                  {client.brandVoiceKeywords ? (
                    client.brandVoiceKeywords.split(',').map((k, i, arr) => (
                      <span
                        key={i}
                        className="inline-block mr-1.5 mb-1 px-2 py-0.5 rounded bg-purple-900/30 border border-purple-700/30 text-purple-300 text-xs"
                      >
                        {k.trim()}
                        {i < arr.length - 1 ? ' ' : ''}
                      </span>
              ...
              <div>
                ...
                <dd className="flex gap-1.5">
                  {client.languages.map(l => (
                    <span
                      key={l}
                      className="px-2 py-0.5 rounded bg-blue-900/30 border border-blue-700/30 text-blue-300 text-xs uppercase"
                    >
                      {l}
                    </span>
                  ))}
        ...
        <div className="bg-gray-900/40 border border-gray-800 rounded-2xl p-5">
          ...
          <div className="space-y-2">
            {(
              ...
              return (
                <div
                  key={p.platform}
                  className="flex items-center gap-3 p-2.5 rounded-lg bg-gray-950/40 border border-gray-800"
                >
          ...
          {socialAccounts.length === 0 && (
            <p className="text-[11px] text-gray-600 mt-3">
              ...
              <Link
                href={`/clients/${client.id}/connections`}
                title="Ouvrir les connexions de ce client"
      ...
      <div className="bg-gray-900/40 border border-gray-800 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          {clientPosts.length > 0 && (
            <Link
              href={`/plan?client=${client.id}`}
              title="Voir tous les posts de ce client dans l'historique"
        ...
        {recentPosts.length === 0 ? (
          <div className="text-center py-8 text-gray-500 text-sm">
            ...
            <div className="mt-2">
              <Link
                href={`/studio?client=${client.id}`}
                title={`Créer le premier post pour ${client.name}`}
                className="inline-flex items-center gap-1.5 text-purple-400 hover:underline"
              >
                <Sparkles className="w-4 h-4" />
      ...
      <div className="bg-gray-900/40 border border-gray-800 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          {clientJobs.length > 0 && (
            <Link
              href="/agents"
              title="Voir l'activité globale de tous les agents IA"
              className="text-xs text-purple-400 hover:underline"
            >
              Tous les jobs →
            </Link>
...
function StartupStep({
  ...
  return (
    <Link
      ...
      className={`rounded-xl border p-3 transition-all duration-150 active:scale-[0.98] ${
        done
...
function RecentPostRow({ post }: { post: Post }) {
  ...
  const Icon = cfg.icon
  const when = new Date(post.createdAt).toLocaleDateString('fr-FR', {
    day: '2-digit',
    month: 'short',
  })
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm text-white truncate">
          {post.brief || post.caption.substring(0, 60)}
        </div>
        <div className="text-[11px] text-gray-500">
          {post.platforms.join(' + ')} · {when}
        </div>
      </div>
...
function AgentJobRow({ job }: { job: AgentJob }) {
  ...
  const when = new Date(job.startedAt).toLocaleDateString('fr-FR', {
    day: '2-digit',
    month: 'short',
    hour: '2-digit',
    minute: '2-digit',
