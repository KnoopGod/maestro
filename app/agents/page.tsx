    ...
    <Link href={`/agents/jobs/${job.id}${backParam}`} className="block group">
      <div className={`bg-gray-900/40 border rounded-xl p-4 hover:border-purple-700/50 transition-all ${
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <div className="text-[11px] text-gray-400 truncate max-w-[200px]">{job.briefSummary}</div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between text-[11px] text-gray-500">
          ...
          <span className="flex items-center gap-1 text-purple-400 group-hover:text-purple-300 transition-all duration-150">
            Voir détail <ArrowRight className="w-3 h-3 group-hover:translate-x-1 transition-transform duration-150" />
          </span>
        </div>
      </div>
    </Link>
    ...
    <article className={`rounded-2xl border p-5 ${isActive ? 'bg-gray-900/40 border-gray-800' : 'bg-gray-900/20 border-gray-800/60 opacity-75'}`}>
      <header className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">🧠 {agent.model}</p>
        </div>
      </header>
      ...
      {agent.seniorPersona && (
        <div className="mb-3 border-t border-gray-800 pt-3 text-[11px] leading-relaxed text-gray-400">
          <div className="text-[11px] uppercase tracking-wider text-gray-500 mb-1">Expertise senior</div>
      ...
      <div className="grid grid-cols-1 gap-2 text-[11px]">
        <div>
          <div className="text-[11px] uppercase tracking-wider text-gray-500 mb-1">Entrées</div>
        ...
        <div>
          <div className="text-[11px] uppercase tracking-wider text-gray-500 mb-1">Sorties</div>
          <div className="flex flex-wrap gap-1">
...
function CronRow({ exec, referenceTs }: { exec: CronExecution; referenceTs: number }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2.5 border-b border-gray-800/60 last:border-0 hover:bg-gray-800/20 transition-colors">
      ...
      <span className={`text-[11px] border rounded-full px-2 py-0.5 flex-shrink-0 ${cfg.badge}`}>{cfg.label}</span>
      ...
      <span className="text-[11px] text-gray-400 flex-shrink-0 w-12 text-right">
        {exec.durationMs != null ? elapsed(exec.durationMs) : '…'}
      </span>
      <span className="text-[11px] text-gray-500 flex-shrink-0 w-16 text-right">
    ...
    <div className="space-y-8">
      ...
      <section aria-labelledby="activity-heading">
        ...
        {runningJobs.length > 0 && (
          <div className="mb-4 space-y-2">
            <p className="text-[11px] uppercase tracking-wider text-gray-500 px-1">En cours</p>
        ...
        {errorJobs.length > 0 && (
          <div className="mb-4 space-y-2">
            <p className="text-[11px] uppercase tracking-wider text-gray-500 px-1">Attention requise</p>
        ...
        {todayJobs.length > 0 && (
          <div className="mb-4 space-y-2">
            <p className="text-[11px] uppercase tracking-wider text-gray-500 px-1">Terminé aujourd&apos;hui ({todayJobs.length})</p>
        ...
        {recentJobs.length > 0 && (
          <div className="space-y-2">
            <p className="text-[11px] uppercase tracking-wider text-gray-500 px-1">Historique récent</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
      ...
      <section aria-labelledby="cron-heading">
        <h2 id="cron-heading" className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
          ...
          Cron / Scheduler
          <span className="text-[11px] text-gray-500 font-normal ml-1">12 dernières exécutions</span>
        </h2>
          ...
          {cronExecs.length === 0 ? (
            ...
            <>
              <div className="flex items-center gap-3 px-4 py-2 border-b border-gray-800 bg-gray-900/60">
                ...
                <div className="flex-1 text-[11px] uppercase tracking-wider text-gray-500">Job</div>
                <div className="text-[11px] uppercase tracking-wider text-gray-500 flex-shrink-0">Statut</div>
                <div className="text-[11px] uppercase tracking-wider text-gray-500 flex-shrink-0 w-16 text-right">Traités</div>
                <div className="text-[11px] uppercase tracking-wider text-gray-500 flex-shrink-0 w-12 text-right">Durée</div>
                <div className="text-[11px] uppercase tracking-wider text-gray-500 flex-shrink-0 w-16 text-right">Quand</div>
              </div>
              ...
              return (
                <div key={step} className="flex items-center gap-2 flex-shrink-0">
                  <div className="bg-gray-950/50 border border-gray-800 rounded-lg p-2.5 min-w-[140px]">
                    <div className="text-[11px] text-purple-400 mb-1">ÉTAPE {step}</div>
                    {stepAgents.length > 0
                      ? stepAgents.map(a => (
