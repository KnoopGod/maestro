# CODEX_SPECS/007 — Guide connexion Meta intégré dans l'app

## Context

La page `/clients/[id]/connections` contient actuellement un `MetaPreflightChecklist` basique (4 points).
Il faut le remplacer par un guide complet et interactif couvrant les 3 phases : prérequis, app developer, token + connexion.

Ce guide doit être visible directement dans Maestro — pas un PDF externe, pas un README.

---

## Goal

Remplacer `components/clients/MetaPreflightChecklist.tsx` par un nouveau composant `MetaSetupGuide`
qui affiche un guide interactif en accordion organisé en 3 phases avec cases à cocher.

---

## Files to modify

### 1. Remplacer `components/clients/MetaPreflightChecklist.tsx` entièrement
...
- `app/clients/[id]/connections/page.tsx` — importe déjà `MetaPreflightChecklist`, le nom reste identique
- `components/clients/MetaConnectionWizard.tsx` — ne pas toucher
- `lib/agents/meta-publisher.ts` — ne pas toucher
...
```
feat: remplacer MetaPreflightChecklist par guide Meta interactif complet (3 phases)
```
