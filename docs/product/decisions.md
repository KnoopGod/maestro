...
**Date** : 2026-06 (audit Phase 0)
**Décision** : Les nouveaux tokens sont chiffrés avec AES-256-GCM quand `MAESTRO_ENCRYPTION_KEY` est configuré.
**Raison** : Réduire le risque de fuite de base avant l'ouverture à des collaborateurs.
**Suite** : définir la clé en production puis reconnecter/migrer les anciens tokens.

### DT-07 — Protection CSRF pragmatique en V1
**Date** : 2026-06 (audit Phase 0)
**Décision** : Cookie `sameSite=strict` + validation `Origin` sur les méthodes mutantes.
**Risque** : Acceptable en mono-admin.
**Suite** : ajouter un token CSRF dédié avant une vraie V2 multi-utilisateur.
