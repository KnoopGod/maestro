...
export async function createUserSession(
  ...
  const row = await queryOne<UserSessionRow>(
    `INSERT INTO user_sessions (id, user_id, expires_at, created_at, ip, user_agent)
     VALUES (?, ?, ?, ?, ?, ?) RETURNING *`,
    [id, userId, expiresAt, now, options?.ip ?? null, options?.userAgent ?? null]
  )
  if (!row) throw new Error('Failed to create user session')
  return mapRow(row)
}
...
export async function deleteUserSession(id: string): Promise<void> {
  await queryOne(`DELETE FROM user_sessions WHERE id = ?`, [id])
}

export async function deleteUserSessionsByUserId(userId: string): Promise<void> {
  await queryOne(`DELETE FROM user_sessions WHERE user_id = ?`, [userId])
}
